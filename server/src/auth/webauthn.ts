...
import { webauthnCeremony } from "../metrics.js";
...
// Runs a WebAuthn ceremony step inside its own span and records the ceremony
// counter on every exit — `ok` on success, the WebAuthnError reason on a known
// failure, `error` on anything unexpected.
function ceremony<T>(op: string, fn: (span: Span) => Promise<T>): Promise<T> {
  return tracer.startActiveSpan(`webauthn.${op}`, async (span) => {
    try {
      const result = await fn(span);
      webauthnCeremony.add(1, { op: op.replace(".", "_"), outcome: "ok" });
      return result;
    } catch (err) {
      const outcome = err instanceof WebAuthnError ? err.reason : "error";
      webauthnCeremony.add(1, { op: op.replace(".", "_"), outcome });
...
export class WebAuthnProvider implements AuthProvider {
  ...
  async beginRegister(userHandle?: UserId): Promise<{ serverNonce: string; options: unknown }> {
    return ceremony("register.begin", async (span) => {
      span.setAttribute("webauthn.rp_id", this.rp.id);
      if (userHandle) span.setAttribute("webauthn.user_handle", userHandle);
  ...
  async finishRegister(
    ...
    return ceremony("register.finish", async (span) => {
      span.setAttribute("webauthn.rp_id", this.rp.id);
      ...
      span.setAttributes({ "webauthn.credential_id": cred.id, "user.id": userIdB64 });
  ...
  async beginAssert(userHandle?: UserId): Promise<{ serverNonce: string; options: unknown }> {
    return ceremony("assert.begin", async (span) => {
      span.setAttribute("webauthn.rp_id", this.rp.id);
      if (userHandle) span.setAttribute("webauthn.user_handle", userHandle);
  ...
  async verifyAssert(
    ...
    return ceremony("assert.finish", async (span) => {
      span.setAttribute("webauthn.rp_id", this.rp.id);
      if (!this.sessions.consume(serverNonceB64)) throw new WebAuthnError("nonce_invalid_or_used");
      ...
      span.setAttribute("webauthn.credential_id", credId);
      ...
      span.setAttribute("webauthn.counter.old", stored.counter);
      ...
      span.setAttribute("webauthn.counter.new", stored.counter);
